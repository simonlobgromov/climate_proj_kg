...
{'long_name': 'Daily 1km Land-surface Temperature',
 ...
 '_FillValue': 0}
```
Где каждому значению (пикселю) соответствует территория площадью $1 km^2$, и если значение пикселя лежит в диапазоне значений $ T \in [7500, 65535]$, то мы говорим о том, что мы для данного пикселя можем определить значение температуры в цельсиях. 
